function create_uncertainty_greedy_results(N_samples, feature_based_func_type, gamma, noise_level, threshold_zero_flag, feedback_model, verbose, N_bins)
%% this script runs the learning scenario where the uncertainty sampling objective is optimized using the greedy algorithm. 
...
mu_0 = zeros(N_dim,1);
sigma_0 = eye(N_dim);
method = 'uncertainty'; % random or uncertainty
%feedback_model = 1;
...
if feedback_model == 1
    ...
    uncertainty_greedy_result_file = ['UnctyGrdy_Res_Nsmps_', num2str(N_samples), '_FeaFncTp_', num2str(feature_based_func_type), '_ga_', num2str(gamma),'_NsLv_', num2str(noise_level), ...
    '_Thr0Flg_', num2str(threshold_zero_flag),'_FdMdl_', num2str(feedback_model),'.mat'];
end
...
load([data_dir, rand_result_file]); % need to run the random result first. 
...
for idx = 1:14
    ...
    for jdx = 1:N_samples
        ...
        elseif strcmp(method, 'uncertainty') == 1
            w_mediate = zeros(N_dim,1);
            alpha = 1;
            SummarySet = GreedyCardinalityConstrained_FeatureFunctionUCB(Data_Mat', feature_based_func_type, w_mediate, (sigma), alpha, V, K, gamma); 
        ...
        if strcmp(method, 'uncertainty') == 1
