function create_uncertainty_DS_optimization_results(N_samples, feature_based_func_type, gamma, noise_level, threshold_zero_flag, feedback_model, verbose, N_bins)
%% this script runs the learning scenario where the uncertainty sampling objective is optimized using the DS optimization algorithm. 
...
method = 'uncertainty_DS'; % random or uncertainty
...
if feedback_model == 1
    ...
    uncertainty_DS_result_file = ['UnctyRandGrdy_Res_Nsmps_', num2str(N_samples), '_FeaFncTp_', num2str(feature_based_func_type), '_ga_', num2str(gamma),'_NsLv_', num2str(noise_level), ...
    '_Thr0Flg_', num2str(threshold_zero_flag),'_FdMdl_', num2str(feedback_model),'.mat'];
...
for idx = 1:14
    ...
    for jdx = 1:N_samples
        ...
        elseif strcmp(method, 'uncertainty_DS') == 1
            w_mediate = zeros(N_dim,1);
            alpha = 1;
            SummarySet = DSoptimized_CardinalityConstrained_FeatureFunctionUCB(Data_Mat', feature_based_func_type, w_mediate, (sigma), alpha, V, K, gamma);             
        ...
        if strcmp(method, 'uncertainty_DS') == 1
            Uncertainty_Queried_Rouge_Score(jdx) = y;
        end
        X_mat = [X_mat;x'];
        Y = [Y;y];
...
title(uncertainty_DS_result_file);
hold off;



save([data_dir, uncertainty_DS_result_file], 'Collected_Uncertainty_Result_Mat', 'Collected_Uncertainty_Queried_Summaries', 'Collected_Uncertainty_Queried_Rouge_Score');
